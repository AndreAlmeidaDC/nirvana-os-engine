...
switch (cmd) {
  case "version": case "--version": case "-v": case "-V": {
    ...
    const edition = existsSync(ef) ? readFileSync(ef, "utf8").trim() : "Nirvana-OS";
  ...
  case "uninstall": {
    if (rest[0] && !rest[0].startsWith("-")) {
      // Paid pack (Track A) takes precedence over the asset registry (Track B).
  ...
  case "embeddings": case "embedder": runScript(join(H, "embeddings.ts"), rest);
  case "gate": {
  ...
  case "update-check": runScript(join(H, "update-check.ts"), rest.length ? rest : ["--status"]);
