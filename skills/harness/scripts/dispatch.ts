...
if (pendingCascade?.kind === "squad-only") {
  ...
  for (const sq of squads) {
    const r = spawnSync("bun", [briefSquadScript, sq, brief, "--project", pid], { encoding: "utf8", env: prepScriptEnv, windowsHide: true });
    if (r.status !== 0) {
...
const r1 = spawnSync("bun", args, { encoding: "utf8", env: prepScriptEnv, windowsHide: true });
if (r1.status !== 0) {
...
const r2 = spawnSync("bun", buildArgs, { encoding: "utf8", env: prepScriptEnv, windowsHide: true });
if (r2.status !== 0) {
...
if (wantExec) {
  ...
  if (businessCanaryDecision.enabled) {
    ...
    const employeePromptFor = (briefFile: string, candidateRoot: string): string => {
      const built = spawnSync("bun", [employeePrompt, slug, intake, projDir, briefFile, candidateRoot], { encoding: "utf8", env: prepScriptEnv, windowsHide: true });
